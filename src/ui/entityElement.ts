import EntityWindow from './components/EntityWindow.ts'
...
import { onClickNotDrag } from '../util/makeDraggable.ts'
...
function makeHover (
  ...
  element.addEventListener('pointerenter', (e) => {
    const info = component.newComponent(EntityWindow, entity)
    makeWindow(info, {
      pointerEvents: false,
      maxSize: 33,
    })
...
function makePopup (component: Component, entity: Entity, e: MouseEvent) {
  ...
  if (!popup) {
    popup = component.newComponent(EntityWindow, entity, true)
    makeWindow(popup, {
      exitButton: true,
      draggable: true,
      resizable: true,
