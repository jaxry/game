...
export function setupFocusEvents (
  ...
  windowElement.addEventListener('pointerleave', () => {
    ...
    // setTimeout allows the pointerenter event to fire before removing windows
    setTimeout(() => {
      for (const window of ancestorWindows(component)) {
        if (!isFocused.has(window) && exitOnPointerLeave.has(window)) {
          // queueMicrotask allows the for loop to finish before actually
          // removing the windows and interrupting the iterator
          queueMicrotask(() => window.remove())
        }
      }
    })
  })
