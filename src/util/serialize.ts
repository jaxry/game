import { makeOrGet, map } from './util'
import { Constructor } from './types'
import { isArray, isPrimitive } from './is'

// Serializes components complex object into components JSON string.
...
// To serialize components class, pass it to the 'serializable' function

interface SerializableOptions<T = any> {
  // Maps an object key's value to components user-defined output before serialization
...
const idToConstructor = new Map<number, object>()
const constructorToId = new WeakMap<object, number>()
...
export function serializable<T> (
  constructor: object, options?: SerializableOptions<T>) {
...
export function serialize (toSerialize: any) {
  ...
  function prepare (value: any, makeSharedReference = true): any {
    ...
    if (value instanceof Set) {
      return {
        $s: map(value, x => prepare(x)),
      }
    } else if (value instanceof Map) {
      return {
        $m: map(value, ([key, x]) => [prepare(key), prepare(x)]),
      }
    } else if (isArray(value)) {
      return map(value, x => prepare(x))
    } else if (value.constructor === Object) {
      return prepareObjectLiteral(value)
    } else if (constructorToId.has(value.constructor)) {
      return prepareInstance(value)
    } else if (constructorToId.has(value)) {
  ...
  const toStringify = {
    ...
    shared: map(sharedObjects.usedSharedObjects,
...
export function deserialize (json: string) {
  ...
  function revive (value: any, revived?: any) {
    ...
    } else if (revived.constructor === Object || isArray(revived)) {
...
function instantiateFromTemplate (object: any) {
  if (object.$i) {
    const Constructor = idToConstructor.get(object.$i) as Constructor
...
function getTransformToFn (object: any, key: string) {
  ...
  return isArray(transform) ? transform[0] : transform
}
...
function getTransformFromFn (object: any, key: string) {
  const transform = constructorToTransform.get(object.constructor)?.[key]
  return isArray(transform) ? transform[1] : undefined
}

function addInheritedProperty<T, K> (
  constructor: object,
  constructorToProp: WeakMap<object, K>,
...
function findParentInMap<T> (
  constructor: object, map: WeakMap<object, T>) {
...
class SharedObjects {
  ...
  find (object: any) {
    ...
    if (object instanceof Set || object instanceof Array) {
      ...
      // or classes without components serializable call
...
// We can grid each property to components single character,
